 ...
 * The routing protocol used here is similar to <a
...
public class FlowRouter extends Router
{
    ...
    protected synchronized void processEvent(Sim_event ev)
    {
    	...
    	System.out.println(super.get_name() +
            ".processEvent(): ev.get_tag() is " + ev.get_tag());
    	System.out.println(super.get_name() +
            ".processEvent(): ev.get_src() is " + ev.get_src());
        *************/
        ...
        switch ( ev.get_tag() )
        {
            ...
            case GridSimTags.JUNK_PKT:
            	//System.out.println(super.get_name() +
                //    ".processEvent(): processNetPacket() + at time = " + GridSim.clock());
                processFlowPacket(ev, ev.get_tag());
                break;
            ...
            case GridSimTags.INSIGNIFICANT:
            	//System.out.println(super.get_name() +
                //  ".processEvent(): processInternalEvent() + at time = " + GridSim.clock());
     ...
     * @post $none
     */
    private synchronized void processFlowPacket(Sim_event ev, int tag)
    {
        ...
        //System.out.println(super.get_name() +
        //    ".processNetPacket() check sched Time now " + GridSim.clock());

        // if no packets at the moment
        if (sched.size() == 0)
        {
            /******  // NOTE: debugging information
            System.out.println(super.get_name() + ".processNetPacket() #packets " +
            		" size " + pkt.getSize()*NetIO.BITS + " baud sched "
                    + sched.getBaudRate() + " largest double " + Double.MAX_VALUE);

            System.out.println(super.get_name() +
                ".processNetPacket() set internal event to fire");
            ******/

            // Fire internal event without delay (delay is incurred entirely at
            // source of Flow
            sendInternalEvent(GridSimTags.SCHEDULE_NOW, sched);
        }

        //System.out.println(super.get_name() +
        //    ".processNetPacket() done sched Time now " + GridSim.clock());

        // log / record ....
        if (super.reportWriter_ != null)
        {
            super.write("");
            super.write("receive incoming, " + pkt + ", delay, " + 0.0);
        }

        // put the actual packet into the last one and resize it accordingly
        pkt.setLast(id);
        pkt.setSize(pkt.getSize());
    ...
    protected synchronized void advertiseHosts()
    {
        ...
        super.sim_pause(2);   // wait for 5 secs to gather the results
    }
